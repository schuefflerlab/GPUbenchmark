...
def plot_and_save_boxplot(data1, data2,data3, save_path):
    fig, ax = plt.subplots()
    ax.boxplot([data1, data2,data3])

    ax.set_xticklabels(['RTX3090', 'A4500','H100'])
    ax.set_ylabel('Time / [s]')
    ax.set_title('Training Time on Mnist per GPU')
    bot,top = ax.get_ylim()
    #print(bot)
